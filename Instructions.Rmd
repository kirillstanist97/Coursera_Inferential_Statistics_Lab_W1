...
#MYCODE
samp1 %>% 
  summarise(samp_mean=mean(area),samp_median=median(area),samp_SD=sd(area),samp_IQR=IQR(area),samp_Q1=quantile(area,0.25),samp_Q3=quantile(area,0.75))

ggplot(data=samp1, aes(area)) + geom_histogram(binwidth = 250)
...
sample_means50 %>% summarise(sampling_dist_mean=mean(x_bar),standard_error=sd(x_bar))
...
ggplot(data = sample_means_small,aes(x=x_bar))+geom_histogram(binwidth = 50)
...
sample_means_small %>% summarise(number_of_observations=n())
...
```{r price-sample}
#My code
Samp2_price_50 <- ames %>% sample_n(size= 50) 

Samp2_price_50 %>% summarise(mean_price=mean(price), SD_price=sd(price), meadian_price=median(price), Q1=quantile(price, 0.25),Q3=quantile(price, 0.75))
...
```{r price-sampling}
Sample_means_price50 <- ames %>% rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>% summarise(sampling_mean=mean(price))

ggplot(data=Sample_means_price50, aes(x=sampling_mean)) + geom_histogram(binwidth = 250)

Sample_means_price50 %>% summarise(mean_distr=mean(sampling_mean), SE=sd(sampling_mean))
...
Sample_means_price150 <- ames %>% rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>% summarise(sampling_mean=mean(price))

ggplot(data=Sample_means_price150, aes(x=sampling_mean)) + geom_histogram(binwidth = 250)

Sample_means_price150 %>% summarise(mean_distr=mean(sampling_mean), SE=sd(sampling_mean))
...
#My code
Sample_price_15 <- ames %>% sample_n(size=15)

Sample_price_15 %>% summarise(sample_mean=mean(price))
...
sample_means15_price <- ames %>% rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>% summarise(sample_means=mean(price))

ggplot(data = sample_means15_price, aes(x=sample_means)) + geom_histogram(binwidth = 250)

sample_means15_price %>% summarise(sampling_dist_mean=mean(sample_means), SE=sd(sample_means))
...
#MY code
sample_means150_price <- ames %>% rep_sample_n(size=150, reps = 2000, replace = TRUE) %>% summarise(sampling_mean=mean(price))

ggplot(sample_means150_price, aes(x=sampling_mean))+geom_histogram(binwidth = 150)

sample_means150_price %>% summarise(sam_dist_mean=mean(sampling_mean), SE=sd(sampling_mean))
